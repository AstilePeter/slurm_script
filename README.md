...
```
sudo groupadd -u 5500 munge

sudo useradd -m munge -u 5600 -g munge

sudo groupadd -u 5501 slurm

sudo useradd -m munge -u 5601 -g slurm
```
...
#
# COMPUTE NODES
NodeName=slave1-desktop NodeAddr=192.168.1.104 CPUs=4 State=UNKNOWN
NodeName=slave2-desktop NodeAddr=192.168.1.101 CPUs=4 State=UNKNOWN
...
```
# TO RUN A JOB
- To run a job use the sbatch command to run the script:


```
sbatch <scriptname>
```
- If we want to use a particular node we can use the option given below in example

```
sbatch --nodelist=<nodename> <scriptname>
```
- To get the list of running jobs use the command:
